<div>
    <div class="flex flex-wrap items-center justify-between gap-3 mb-6">
        ...
        <div class="flex flex-wrap items-center gap-2">
            ...
            <flux:input wire:model.live.debounce.300ms="search" icon="magnifying-glass" placeholder="Cari produk..."
                class="w-full sm:w-64" clearable />
                ...
                @forelse ($this->products as $product)
                    <flux:table.row wire:key="product-{{ $product->id }}">
                        <flux:table.cell>
                            ...
                            <div class="text-sm text-zinc-500 line-clamp-1 max-w-sm">{{ $product->description ?? '-' }}
                            </div>
                        </flux:table.cell>
                        <flux:table.cell>
                        ...
                        <flux:table.cell colspan="3" class="p-0">
                            <div class="flex flex-col items-center justify-center py-20 whitespace-normal">
                                @if($status === 'active')
                                    ...
                                    <flux:button wire:click="$dispatchTo('product.form', 'load-product-form')"
                                        x-on:click="$flux.modal('product-form-modal').show()" variant="primary"
                                        icon="squares-plus" class="shadow-sm">Tambah Produk</flux:button>
                                @else
                                    <div
                                        class="p-4 rounded-full bg-zinc-100 dark:bg-zinc-800/50 outline outline-1 outline-zinc-200 dark:outline-zinc-700/50 mb-5 text-zinc-400 dark:text-zinc-500">
                                    ...
                                    <flux:text class="text-zinc-500 dark:text-zinc-400 text-center max-w-sm">Data produk yang
                                        Anda
    ...
    <flux:modal name="product-delete-modal" class="w-full md:w-[32rem]">
        <div class="mb-6">
    ...
    <flux:modal name="product-force-delete-modal" class="w-full md:w-[32rem]">
        <div class="mb-6">
