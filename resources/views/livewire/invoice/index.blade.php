<div>
    <div class="flex flex-wrap items-center justify-between gap-3 mb-6">
        ...
        <div class="flex flex-wrap items-center gap-2">
            ...
            <flux:input wire:model.live.debounce.300ms="search" icon="magnifying-glass"
                placeholder="Cari nomor/klien..." class="w-full sm:w-64" clearable />

            @if($status !== 'trashed')
                <flux:button wire:click="$dispatchTo('invoice.form', 'load-invoice-form')"
                    x-on:click="$flux.modal('invoice-form-modal').show()" variant="primary" icon="document-plus">Buat
                    Invoice</flux:button>
            @endif
        </div>
    </div>
                    ...
                    <flux:table.row wire:key="invoice-{{ $invoice->id }}">
                        ...
                        <flux:table.cell>
                            <div class="text-sm font-medium text-zinc-700 dark:text-zinc-300">
                                {{ $invoice->creator->name ?? 'System' }}</div>
                        </flux:table.cell>
                        <flux:table.cell>
                            @php
                        ...
                        <flux:table.cell>
                            <div class="text-sm font-semibold">Rp {{ number_format($invoice->grand_total, 2, ',', '.') }}
                            </div>
                        </flux:table.cell>
                        <flux:table.cell>
                                ...
                                <flux:menu>
                                    @if($status !== 'trashed')
                                        <flux:menu.item icon="eye"
                                            wire:click="$dispatchTo('invoice.show', 'load-invoice-show', { id: {{ $invoice->id }} })"
                                            x-on:click="$flux.modal('invoice-show-modal').show()">Lihat Detail</flux:menu.item>
                                        @if(!in_array($invoice->status->value ?? $invoice->status, ['paid', 'canceled']))
                                            <flux:menu.item icon="pencil-square"
                                                wire:click="$dispatchTo('invoice.form', 'load-invoice-form', { id: {{ $invoice->id }} })"
                                                x-on:click="$flux.modal('invoice-form-modal').show()">Edit</flux:menu.item>
                                            <flux:menu.separator />
                                            <flux:menu.item icon="trash" variant="danger"
                                                wire:click="confirmDelete({{ $invoice->id }})"
                                                x-on:click="$flux.modal('invoice-delete-modal').show()">Hapus</flux:menu.item>
                                        @endif
                                    @else
                                        <flux:menu.item icon="arrow-path" wire:click="restore({{ $invoice->id }})">Pulihkan
                                        </flux:menu.item>
                                        <flux:menu.item icon="trash" variant="danger"
                                            wire:click="confirmForceDelete({{ $invoice->id }})"
                                            x-on:click="$flux.modal('invoice-force-delete-modal').show()">Hapus Permanen
                                        </flux:menu.item>
                                    @endif
                                </flux:menu>
                            </flux:dropdown>
                        ...
                        <flux:table.cell colspan="6" class="p-0">
                            <div class="flex flex-col items-center justify-center py-20 whitespace-normal">
                                @if($status !== 'trashed')
                                    ...
                                    <flux:text class="text-zinc-500 dark:text-zinc-400 text-center mb-6 max-w-sm">Data tagihan
                                        tidak ditemukan. Buat invoice baru untuk menagih klien Anda.</flux:text>
                                    <flux:button wire:click="$dispatchTo('invoice.form', 'load-invoice-form')"
                                        x-on:click="$flux.modal('invoice-form-modal').show()" variant="primary"
                                        icon="document-plus" class="shadow-sm">Buat Invoice</flux:button>
                                @else
                                    <div
                                        class="p-4 rounded-full bg-zinc-100 dark:bg-zinc-800/50 outline outline-1 outline-zinc-200 dark:outline-zinc-700/50 mb-5 text-zinc-400 dark:text-zinc-500">
                                    ...
                                    <flux:text class="text-zinc-500 dark:text-zinc-400 text-center max-w-sm">Data invoice yang
                                        dibuang akan disembunyikan di sini.</flux:text>
                                @endif
                            </div>
                        </flux:table.cell>
                    </flux:table.row>
        ...
        @forelse ($this->invoices as $invoice)
            ...
            <div class="rounded-xl border border-zinc-200 dark:border-zinc-700 bg-white dark:bg-zinc-900 p-4"
                ...
                <div class="flex items-center justify-between mt-3 pt-3 border-t border-zinc-100 dark:border-zinc-800">
                    <div class="text-xs text-zinc-400">Tempo: {{ $invoice->due_date->format('d M Y') }}</div>
                    <div class="text-sm font-semibold text-zinc-900 dark:text-zinc-100">Rp
                        {{ number_format($invoice->grand_total, 0, ',', '.') }}</div>
                </div>
            </div>
        @empty
            <div class="flex flex-col items-center justify-center py-16 text-center">
                @if($status !== 'trashed')
                    ...
                    <flux:heading size="lg" class="mb-2 dark:text-zinc-200">Belum ada Invoice</flux:heading>
                    <flux:text class="text-zinc-500 dark:text-zinc-400 text-center mb-6 max-w-sm">Data tagihan tidak ditemukan.
                    </flux:text>
                    <flux:button wire:click="$dispatchTo('invoice.form', 'load-invoice-form')"
                        x-on:click="$flux.modal('invoice-form-modal').show()" variant="primary" icon="document-plus"
                        class="shadow-sm">Buat Invoice</flux:button>
                @else
                    <div
                        class="p-4 rounded-full bg-zinc-100 dark:bg-zinc-800/50 outline outline-1 outline-zinc-200 dark:outline-zinc-700/50 mb-5 text-zinc-400 dark:text-zinc-500">
                        <flux:icon name="archive-box" class="size-8" />
    ...
    <flux:modal name="invoice-delete-modal" class="w-full md:w-[32rem]">
        <div class="mb-6">
            <flux:heading size="lg">Konfirmasi Hapus</flux:heading>
            <flux:subheading>Invoice akan dipindahkan ke Sampah. Dokumen yang Lunas atau Dibatalkan tidak akan bisa
                dihapus.</flux:subheading>
        </div>
    ...
    <flux:modal name="invoice-force-delete-modal" class="w-full md:w-[32rem]">
        <div class="mb-6">
            <flux:heading size="lg">Hapus Permanen</flux:heading>
            <flux:subheading>Tindakan ini tidak dapat dibatalkan. Riwayat invoice ini akan menghilang total.
            </flux:subheading>
        </div>
