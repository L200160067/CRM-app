<div>
    <flux:modal name="invoice-show-modal" class="md:w-[50rem]">
        @if($invoice)
            ...
            <div class="mb-5 pb-5 border-b border-zinc-200 dark:border-zinc-700">
                <div class="flex items-start gap-5 mb-5">
                    @if(config('company.logo'))
                        <div
                            class="w-24 h-24 shrink-0 rounded-lg overflow-hidden border border-zinc-200 dark:border-zinc-700 bg-white dark:bg-zinc-900 flex items-center justify-center">
                            <img src="{{ asset(config('company.logo')) }}" alt="Logo" class="w-full h-full object-contain"
                                onerror="this.outerHTML='<svg class=\'w-8 h-8 text-zinc-400\' fill=\'none\' viewBox=\'0 0 24 24\' stroke=\'currentColor\'><path stroke-linecap=\'round\' stroke-linejoin=\'round\' stroke-width=\'2\' d=\'M4 16l4.586-4.586a2 2 0 012.828 0L16 16m-2-2l1.586-1.586a2 2 0 012.828 0L20 14m-6-6h.01M6 20h12a2 2 0 002-2V6a2 2 0 00-2-2H6a2 2 0 00-2 2v12a2 2 0 002 2z\' /></svg>'">
                        </div>
                    @else
                        <div
                            class="w-24 h-24 shrink-0 rounded-lg border border-zinc-200 dark:border-zinc-700 bg-zinc-50 dark:bg-zinc-800 flex items-center justify-center">
                    ...
                    <div class="flex flex-col justify-center py-1">
                        ...
                        @if(config('company.sub_name'))
                            <div class="text-zinc-700 dark:text-zinc-300 font-medium text-sm">{{ config('company.sub_name') }}
                            </div>
                        @endif
                ...
                <div class="text-sm text-zinc-500 flex flex-col gap-1.5">
                    <div class="flex flex-wrap items-center gap-x-2 gap-y-1">
                        <span><span class="font-medium text-zinc-700 dark:text-zinc-300">No. WhatsApp:</span>
                            {{ config('company.phone') }}</span>
                        <span><span class="font-medium text-zinc-700 dark:text-zinc-300">Email:</span>
                            {{ config('company.email') }}</span>
                        {{-- <span class="hidden sm:inline text-zinc-300 dark:text-zinc-600">|</span> --}}
                    </div>
                    <div>
                        <span class="font-medium text-zinc-700 dark:text-zinc-300">Website:</span>
            ...
            <!-- Invoice Code -->
            <div
                class="flex flex-col items-center justify-center mb-10 text-center bg-zinc-50 dark:bg-zinc-800/40 border border-zinc-100 dark:border-zinc-700/50 py-6 rounded-xl">
                <h1 class="text-3xl font-bold tracking-widest text-zinc-900 dark:text-white">{{ $invoice->invoice_number }}
                </h1>
            </div>
            ...
            <div class="grid grid-cols-2 gap-8 mb-8">
                ...
                <div class="space-y-3 text-sm">
                    <div class="flex items-center justify-between">
                        ...
                        <span
                            class="font-medium text-zinc-900 dark:text-zinc-100">{{ $invoice->issue_date->format('d M Y') }}</span>
                    </div>
                    <div class="flex items-center justify-between">
                        <span class="text-zinc-500">Jatuh Tempo</span>
                        <span
                            class="font-medium text-red-600 dark:text-red-400">{{ $invoice->due_date->format('d M Y') }}</span>
                    </div>
                </div>
            </div>
                    ...
                    <tbody class="divide-y divide-zinc-200 dark:divide-zinc-700">
                        @foreach($invoice->items as $item)
                            <tr>
                                ...
                                <td class="p-3 align-top text-right whitespace-nowrap">Rp
                                    {{ number_format($item->unit_price, 2, ',', '.') }}</td>
                                <td class="p-3 align-top text-right font-medium whitespace-nowrap">Rp
            ...
            <div class="flex justify-end mb-8">
                <div class="w-1/2 space-y-3">
                    ...
                    <div
                        class="flex justify-between text-lg font-bold border-t border-zinc-200 dark:border-zinc-700 pt-3 mt-2">
            ...
            @if($invoice->notes)
                <div
                    class="bg-zinc-50 dark:bg-zinc-800/50 p-4 rounded-lg text-sm text-zinc-600 dark:text-zinc-400 whitespace-pre-line">
            ...
            <div class="flex justify-end gap-2 pt-6">
                @if($invoice)
                    <a href="{{ route('invoices.print', $invoice) }}" target="_blank">
                        <flux:button variant="primary" icon="printer">Cetak PDF</flux:button>
                    </a>
                @endif
                <flux:modal.close>
